...
Path("/multi_arinc/3.ORL-A").mkdir(parents=True, exist_ok=True)
...
with open('%s%s' % ('/multi_arinc/3.ORL-A/', filename), 'w') as r:
    for item in sample:
        r.write('%s' % item)

with open('%s%s' % ('/multi_arinc/3.ORL-A/', filename), 'a+') as r:
    for item in test:
        r.write('%s\n' % item)
